...
def one_literal_rule(clause_set):
    for clause_1 in clause_set[:]:  # Iterate over a copy of the list
        if len(clause_1) == 1:
            literal = clause_1[0]
            clause_set.remove(clause_1)
            for clause_2 in clause_set[:]:  # Iterate over a copy of the list
...
def split_method(clause_set):
    clause_set_copy = copy.deepcopy(clause_set)  # Use deepcopy to maintain the original form
    clause = [clause_set[0][0]]
    clause_complement = [complement(clause[0])]
    clause_set.append(clause)
    clause_set_copy.append(clause_complement)
    print(f"Applying the split method, for the first case we add clause {clause} to the clause set and apply DPLL:\n{clause_set}")
    x = DP(clause_set)
    print(f"Applying the split method, for the second case we add clause {clause_complement} to the clause set and apply DPLL:\n{clause_set_copy}")
    y = DP(clause_set_copy)
    if x == True or y == True:
        print("One of the cases is satisfiable.")
        return True
    print("Both cases are unsatisfiable.")
    return False
# applying DP
def DP(clause_set):
    while len(clause_set)!=0:
         if [] in clause_set:
                print("\n")
                return False
         elif one_literal_rule(clause_set):
              ...
              if [] in clause_set:
                    print("\n")
                    return False
         elif split_method(clause_set):
              print(f"The 1st and 2nd rules of DP cannot be applied, but split method can be applied. The clause set is now:\n{clause_set}")
              if [] in clause_set:
                    print("\n")
                    return False
         else:
...
if DP(clause_set):
     ...
     print("Unsatifiable, applying the DP method the empty clause was achieved for all cases.\n",clause_set)
